...
public static class OrtoCameraHelper
{
	public static float UnitsToPixels(Camera camera, float units)
	{
		
		return Mathf.Round( ( Mathf.Abs(units) * (Screen.height/2f)) / camera.orthographicSize);
	}
